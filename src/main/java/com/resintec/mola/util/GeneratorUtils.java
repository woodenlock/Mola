...
public class GeneratorUtils {
	...
	public static void generate(GenrateConfiguration config) {
		if (null == config) {
			throw new IllegalArgumentException("Failed to generate due to empty param:GenrateConfiguration.");
		}

		generate(config.getAuthor(), config.getOverride(), config.getConnection(), config.getDataSource(), config.getPath(), config.getMappings());
	}
	 ...
	 * @param mapping
	 */
	public static void generate(String author, Boolean override, ConnectionConfiguration connection, DataSourceConfiguration dataSource,
			AbstractFilePathConfiguration path, FieldMappingConfiguration... mapping) {
		log.info(
		...
		if (null == connection || CommonUtils.isBlank(connection.getUrl())
				|| CommonUtils.isBlank(connection.getDriverClassFullPath())
			...
			for (Table table : need) {
				...
				for (Field field : fields) {
					...
					if (null != data && data.isOutput()) {
						generateFile(author, table, data, path);
					}
				}
				log.info("Finished to generate files from table:{}.", table.getName());
			}
		} catch (SecurityException | IllegalArgumentException | IllegalAccessException | SQLException | IOException
				| BusinessException e) {
	 ...
	 * @throws IOException
	 */
	public static void generateFile(String author, Table table, SinglePathConfiguration singlePath,
			AbstractFilePathConfiguration allPath) throws IOException {
		if (null == table || CommonUtils.isBlank(table.getEntityName()) || null == singlePath
				|| CommonUtils.isBlank(singlePath.getTargetBasicDirectory())
				|| CommonUtils.isBlank(singlePath.getTargetPackagePath())
				|| CommonUtils.isBlank(singlePath.getTargetExtension())
				|| CommonUtils.isBlank(singlePath.getTemplateFullPath())
				|| CommonUtils.isBlank(singlePath.getTemplateFileName()) || null == allPath) {
		...
		log.info("Succeed in the output of the target file:{}.Target table name:{}.", file.getAbsolutePath(),
				table.getName());
	}
	...
	private static void rollback() {
		...
		for (String string : ROLLBACK_PATHS) {
			...
			if (!act) {
				throw new BusinessException("Failed to rollback target File:" + string
						+ ".Left files needs to be deleteds:" + ROLLBACK_PATHS);
			}
			affected++;
		}
